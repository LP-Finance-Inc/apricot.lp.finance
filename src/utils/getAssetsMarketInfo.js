...
export const getAssetsMarketInfo = async () => {
  ...
  try {
    ...
    console.log(assetPoolLoader);
    const fetchPriceObj = await assetPoolLoader.fetchPrice(
      "BTC"
    );
    console.log("fetchPrice", fetchPriceObj);
    
    const AssetsPoolInfoObj = await assetPoolLoader.getAssetPool(
      "BTC"
    );

    console.log("AssetsPoolInfoObj", AssetsPoolInfoObj)
